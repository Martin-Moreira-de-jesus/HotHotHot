...
class HistoricSorter extends Observer {
    ...
    sort(sortBy, direction) {
        this.updateSortButton(sortBy, direction);
        switch (sortBy) {
            case "temp":
    ...
    updateSortButton(sortBy, direction) {
        document.querySelectorAll("#historic tbody img").forEach(img => {
            img.src = "";
        });

        // pour une raison obscure ca marche pas en concaténant
        let imageSrc = "./images/historic/caret-";
        imageSrc += direction === "asc" ? "up" : "down";
        imageSrc += ".svg";

        switch (sortBy) {
            case "temp":
                document.getElementById("svg-temp").src = imageSrc;
                break;
            case "date":
                document.getElementById("svg-date").src = imageSrc;
                break;
            case "sensor":
                document.getElementById("svg-sensor").src = imageSrc;
                break;
        }
    }
}
